...
pub fn t(key: &str) -> String {
    let (zh, en): (&str, &str) = match key {
        ...
        "harness.core_not_found" => (
            "未找到 MIR3 AI Core，请先完成安装",
            "MIR3 AI Core package not found, run setup first",
        ),
        "harness.manifest_invalid" => (
            "MIR3 AI Core 包清单无效",
            "Invalid MIR3 AI Core package manifest",
        ),
        ...
        "harness.start_failed" => (
            "启动 MIR3 AI Core 服务失败",
            "Failed to start MIR3 AI Core service",
        ),
        "harness.health_unhealthy" => (
            "MIR3 AI Core 服务未就绪",
            "MIR3 AI Core service is not ready",
        ),
