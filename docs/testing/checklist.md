...
- [ ] **首次装配 E2E**：首启自动下载 Node + MIR3 AI Core → 内嵌 WebView 加载 `127.0.0.1:3080`。
...
- [ ] MIR3 AI Core 本地代码执行能力的风险审查与提示验证。
- [ ] 插件来源白名单校验（仅允许预设清单/受信来源）。
- [ ] WebView 跨源通信与 iframe origin 校验。
- [ ] 配置文件/数据目录权限（`~/.mir3-studio-ai`/`~/.mir3-studio-ai.dev`）检查。
