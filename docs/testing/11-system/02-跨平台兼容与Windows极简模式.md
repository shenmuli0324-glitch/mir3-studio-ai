...
[测试类型] 兼容性
[前置条件] Windows 已导入完整 996 项目；启动游戏后任务管理器“详细信息”页显示 x86 `game.exe`，其文件描述为 `Game Module`，完整进程路径可能因游戏保护无法读取
[测试步骤] 1. 打开 GUI Designer，观察顶部游戏状态按钮。2. 保持 `game.exe` 运行并点击状态按钮手动刷新。3. 只保留 `GameCenter.exe`、关闭 `game.exe` 后再次刷新。4. 临时令已导入客户端目录不可规范化，验证状态检测不依赖项目路径解析。5. 模拟进程快照失败并悬停状态按钮
[预期结果] 1. Studio 直接通过 Windows 进程快照识别 `game.exe`，显示「游戏运行中」，不读取受保护的进程路径。2. 只有 `GameCenter.exe` 时显示「游戏未运行」，文件描述 `Game Module` 和游戏窗口标题不被当作独立进程。3. 客户端目录解析异常不阻止进程枚举。4. 真实的进程快照错误显示「游戏检测失败」，按钮提示包含后端错误码。5. 该检测只更新状态灯，不控制窗口、不注入进程，也不授予文件写权限
