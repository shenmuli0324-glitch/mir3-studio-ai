...
- Every completed functional change must produce a runnable native package for physical-device testing after all checks pass. Build the current host platform with `pnpm tauri build` (or an explicitly requested target), verify that the generated app/installer exists and can launch, and report its absolute artifact path in the final handoff.
- On macOS, when no Developer ID certificate is configured, use `APPLE_SIGNING_IDENTITY=- pnpm tauri build` so the local test bundle receives a complete ad-hoc signature; verify both the `.app` and the copy inside the `.dmg` with `codesign --verify --deep --strict`. Ad-hoc signing is only for local physical-device testing and does not replace Developer ID signing/notarization for public distribution.
