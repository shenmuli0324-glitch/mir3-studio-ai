...
- Every completed functional change must produce a runnable native package for physical-device testing after all checks pass. On macOS, the single canonical delivery command is `pnpm package:mac`; do not assemble ad-hoc `tauri build`, signing, DMG verification, size, or SHA-256 commands manually unless debugging the packaging script itself. Report the absolute artifact paths from this command in the final handoff.
- `pnpm package:mac` owns Apple Silicon `.app`/`.dmg` creation and verification. When no Developer ID certificate is configured it applies a complete ad-hoc signature, verifies the app with `codesign --verify --deep --strict`, validates the DMG with `hdiutil verify`, and prints artifact sizes and SHA-256. Ad-hoc signing is only for local physical-device testing and does not replace Developer ID signing/notarization for public distribution.
- Packaging is part of completion, not an optional follow-up. If packaging or launch verification fails, do not describe the development task as complete; report the blocking command, error, and any platform/signing limitation. Never commit generated bundle artifacts.
