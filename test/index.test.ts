...
describe('studio shell contract', () => {
  ...
  it('uses one scoped domain workspace and Harness bridge for every system', () => {
    ...
    const workbookEditor = readFileSync(new URL('../src/features/devtools/domain/domain-workbook-editor.tsx', import.meta.url), 'utf8')
    const appLayout = readFileSync(new URL('../src/layout/index.tsx', import.meta.url), 'utf8')
    const persistentDevtools = readFileSync(new URL('../src/layout/components/persistent-devtools.tsx', import.meta.url), 'utf8')
    const pluginSettings = readFileSync(new URL('../src/components/config-plugin.tsx', import.meta.url), 'utf8')
    const domainPackManager = readFileSync(new URL('../src/components/domain-pack-manager.tsx', import.meta.url), 'utf8')
    const aiPanel = readFileSync(new URL('../src/features/system-ai/system-ai-panel.tsx', import.meta.url), 'utf8')
    const aiConversationPanel = readFileSync(new URL('../src/features/system-ai/ai-conversation-panel.tsx', import.meta.url), 'utf8')
    const capabilityGovernance = readFileSync(new URL('../src/features/system-ai/capability-governance.tsx', import.meta.url), 'utf8')
    ...
    expect(domainApi).toContain('invoke<DomainWorkingCopy>(\'domain_working_copy_open\'')
    expect(domainApi).toContain('invoke<DomainWorkingSaveResult>(\'domain_working_save\'')
    expect(domainApi).toContain('invoke<DomainSaveNode[]>(\'domain_save_node_list\'')
    expect(domainApi).toContain('invoke<DomainDraftConfirmation>(\'draft_legacy_clone\'')
    expect(domainApi).toContain('invoke<CompositeDraftReview>(\'draft_composite_preview\'')
    expect(domainApi).toContain('invoke<DomainCompositeWorkingSaveResult>(\'domain_working_composite_save\'')
    ...
    expect(domainApi).toContain('invoke<UserCapability>(\'user_capability_compile_global\'')
    expect(domainApi).toContain('invoke<CapabilityResolution[]>(\'user_capability_resolve\'')
    expect(domainApi).toContain('invoke<CapabilityResolution[]>(\'user_capability_versions\'')
    expect(domainApi).toContain('invoke<CapabilityResolution>(\'user_capability_promote\'')
    expect(domainApi).toContain('invoke<UserCapability>(\'user_capability_rollback\'')
    ...
    expect(domainApi).toContain('invoke<DomainPackState[]>(\'domain_pack_list\')')
    expect(pluginSettings).toContain('<DomainPackManager />')
    expect(domainPackManager).toContain('pack.candidate?.version')
    expect(domainPackManager).toContain('rollbackDomainPack')
    expect(domainView).toContain('<SystemAiPanel')
    expect(domainView).toContain('<DomainFileSidebar')
    expect(domainView).toContain('<DirectoryTree')
    expect(domainView).toContain('<FileSourceWorkspace')
    expect(domainView).toContain('<DomainWorkbookEditor')
    expect(domainView).toContain('openDomainWorkingCopy(')
    expect(domainView).toContain('openDomainWorkingText(project!.id, selectedFile!.path, activeWorkingCopyId)')
    expect(domainView).toContain('patchDomainWorkingText(')
    expect(domainView).toContain('saveDomainWorkingCopy(')
    expect(domainView).toContain('restoreDomainSaveNode(')
    expect(workbookEditor).toContain('readOnly={!editable || busy}')
    expect(domainView).toContain('loadDomainWorkingWorkbook(project!.id, selectedFile!.path, activeWorkingCopyId)')
    expect(domainView).toContain('patchDomainWorkingXls(')
    expect(domainView).toContain('workingCopy?.revision ?? 0')
    expect(domainView).toContain('file.ownership === \'owned\' || file.ownership === \'shared\'')
    expect(domainView).toContain('ensureWorkingCopy={ensureWorkingCopy}')
    expect(domainView).not.toContain('<CompactDraftBar')
    ...
    expect(domainView).not.toContain('validateDomainSystem')
    expect(appLayout).toContain('<CompositeDraftReviewDialog')
    expect(appLayout).toContain('<PersistentDevTools mounted={devtoolsMounted} active={activeView === \'devtools\'} target={devtoolsTarget} />')
    expect(appLayout).toContain('if (view === \'devtools\')')
    expect(persistentDevtools).toContain('lazy(() => import(\'@/views/devtools-view\')')
    expect(persistentDevtools).toContain('<Suspense fallback=')
    expect(appLayout).toContain('type: \'mir3/globalSession.resume\'')
    expect(appLayout).toContain('studio.composite_review.reopen')
    expect(aiPanel).toContain('\'mir3/systemSession.create\'')
    expect(aiPanel).toContain('\'mir3/globalSession.create\'')
    expect(aiPanel).not.toContain('$' + '{summary}\\n$' + '{JSON.stringify(structuredContext)}')
    expect(aiPanel).toContain('buildGlobalTaskHandoff({')
    expect(aiPanel).toContain('appendScopedUserRequest(context, content)')
    expect(aiPanel).toContain('<GlobalScopePicker manifest={manifest}')
    expect(aiConversationPanel).toContain('className="size-8 shrink-0 rounded-full bg-accent text-white"')
    expect(aiPanel).not.toContain('<CapabilityGovernance')
    expect(aiPanel).not.toContain('studio.devtools.ai.welcome')
    expect(aiPanel).not.toContain('messages.slice(-8)')
    expect(aiPanel).toContain('const sourceSessionId = sessionId || globalSessionId')
    expect(aiPanel).toContain('explicitSummary: {')
    expect(aiPanel).toContain('completedOperations: taskToolCalls.map(toolCallLabel)')
    expect(aiPanel).toContain('references: {')
    expect(aiPanel).toContain('selectedResource=${selectedResourceId')
    expect(aiPanel).toContain('unfinishedSteps: runningCalls.map(toolCallLabel)')
    ...
    expect(aiPanel).toContain('buildSystemTaskScopeContract(manifest, taskId, activeWorkingCopyId, manifests)')
    expect(aiPanel).toContain('assertSystemTaskScopeLease(issued, contract)')
    expect(aiPanel).toContain('buildSystemTaskRenewalContract(manifest, identity.taskId, previous)')
    expect(aiPanel).toContain('systemTaskSafetyInstructions(manifest)')
    expect(aiPanel).toContain('binding.pluginVersion !== manifest.version')
    expect(aiConversationPanel).toContain('outcome: \'allowed-once\'')
    ...
    expect(aiPanel).not.toContain('activateMemoryCandidate')
    expect(aiPanel).not.toContain('revokeMemoryCandidate')
    expect(aiPanel).toContain('[Relevant task receipts]')
    expect(aiPanel).toContain('[Resolved reusable capabilities]')
    expect(aiPanel).not.toContain('<CapabilityGovernance')
    expect(aiPanel).not.toContain('saveDomainMemory')
    expect(capabilityGovernance).toContain('compileGlobalUserCapability')
    expect(capabilityGovernance).toContain('promoteUserCapability')
    expect(capabilityGovernance).toContain('rollbackUserCapability')
    expect(capabilityGovernance).toContain('setSharedUserCapabilityStatus')
    expect(aiPanel).toContain('activeSessionId = `mir3-system-')
    expect(aiPanel).toContain('resumedSessionRef.current = activeSessionId')
    ...
    expect(iframeShim).toContain('capabilities.globalSession === true')
    expect(iframeShim).toContain('capabilities.ordinarySessionCanary === true')
    expect(iframeShim).toContain('invoke(\'core_mcp_canary_run\')')
    expect(iframeShim).toContain('invoke<Array<{ systemId?: string }>>(\'domain_system_list\')')
    expect(iframeShim).toContain('invoke<boolean>(\'rollback_core_update\')')
    expect(iframeShim).toContain('store.harness.refreshIframe()')
    expect(iframeShim).not.toContain('iframe.src = iframe.src')
    expect(iframeShim).toContain('await invoke(\'launch_harness\')')
    ...
    expect(coreServer).toContain('inject: [\'sessions\', \'sandboxPolicy\', \'fileReferences\']')
    expect(coreServer).toContain('exec?.agent?.session')
    expect(coreServer).toContain('MIR3_SYSTEM_SESSION_DRAFT_REQUIRED')
    expect(coreServer).toContain('!isMir3ManagedSession(session) || previousModes.has(session.id)')
