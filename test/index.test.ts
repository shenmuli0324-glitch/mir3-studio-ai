...
describe('studio shell contract', () => {
  ...
  it('uses one scoped domain workspace and Harness bridge for every system', () => {
    ...
    expect(domainView).toContain('<XlsSourcePreview')
    ...
    expect(domainView).toContain('openDomainXls(project!.id, selectedFile!.path)')
    expect(domainView).toContain('readDomainXlsSheet(project!.id, selectedFile!.path, sheetName, workbook.data!.sha256)')
