...
describe('studio shell contract', () => {
  ...
  it('uses one scoped domain workspace and Harness bridge for every system', () => {
    ...
    expect(domainView).toContain('legacyOrUnscoped')
    expect(domainView).toContain('expectedSources[change.path] = change.baseSha256')
    expect(domainView).toContain('studio.devtools.controls.draft')
    ...
    expect(domainView).toContain('queryDomainResources(project!.id, tool.id, deferredSearch, resourceLimit)')
    expect(domainView).toContain('studio.devtools.resources.load_more')
