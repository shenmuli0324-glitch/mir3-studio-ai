...
describe('studio shell contract', () => {
  ...
  it('uses one scoped domain workspace and Harness bridge for every system', () => {
    ...
    expect(domainApi).toContain('invoke<CompositeDraftApplyResult>(\'draft_composite_apply\'')
    ...
    expect(domainView).toContain('openDomainText(project!.id, selectedFile!.path, activeDraftId)')
    ...
    expect(domainView).toContain('mutationFn: (draftId: string) => validateDomainDraft(')
    expect(domainView).toContain('onSuccess: setValidation')
    ...
    expect(aiPanel).toContain('buildSystemTaskScopeContract(manifest, taskId, draftId, manifests)')
