...
describe('studio shell contract', () => {
  ...
  it('uses one scoped domain workspace and Harness bridge for every system', () => {
    ...
    expect(aiPanel).toContain('formatTaskReceiptSummary(semanticSummary)')
    expect(aiPanel).toContain('toolCalls: semanticSummary.completedOperations ?? []')
    expect(aiPanel).not.toContain('messages.slice(-8)')
    expect(aiPanel).toContain('setError(t(\'studio.devtools.ai.global_needs_session\'))')
    expect(aiPanel).not.toContain('sessionId || \'unbound\'')
    ...
    expect(aiPanel).toContain('<CapabilityGovernance')
