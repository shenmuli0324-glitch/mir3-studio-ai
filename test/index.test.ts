...
describe('studio shell contract', () => {
  ...
  it('uses one scoped domain workspace and Harness bridge for every system', () => {
    ...
    expect(domainApi).toContain('invoke<UserCapability>(\'user_capability_save\'')
    ...
    expect(aiPanel).toContain('type: \'domain-operation\'')
    expect(aiPanel).toContain('operation: officialCapability.id')
    ...
    expect(coreServer).toContain('if (!isSystemSession(session))')
