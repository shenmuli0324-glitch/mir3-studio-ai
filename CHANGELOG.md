...
- 33 个开发工具改用中央官方文件目录生成精确投影；通用关键词和正文命中不再授予文件归属或写权限，缺少可靠官方映射的工具默认保持空白并支持项目私有手动关联。
- Studio 文件树、系统 AI 与 MIR3 MCP 共用 `systemId + bindingId + scope` 授权链，按整文件、XLS 工作表/列或脚本区段校验，跨系统访问、越界写入和过期版本均失败关闭。
- 二开项目只扫描活动运行目录并一次生成 SQLite 文件—系统投影；打开工具直接按系统索引查询，关联参考按需加载，万文件索引性能测试稳定在秒级。
- 同一路径继续共用项目级 Working Copy、revision、SHA 冲突保护和原子保存节点；手动关联采用增量投影更新，不再触发整个项目重扫。

## 0.9.32 - 2026-09-07

- 项目导入只确认`客户端`和`引擎`身份，项目列表固定按首次导入时间倒序；切换项目、选择工作区和重新扫描不再改变顺序。
- 二开索引和 MIR3 受管 Harness 会话跳过运行文件、加密包、未知格式及损坏或伪装的 XLS；普通 Harness 工作台保持原能力，Core Plugin 升级至 1.3.5。
- BIFF8 `.xls` 以可编辑表格打开，读取并检索全部工作表和有效单元格；人工与 AI 共用 Working Copy，保存继续写回原 `.xls` 文件并保留未修改记录和单元格样式。
- Working Copy 永久保留首次磁盘基线；游戏或外部工具修改同一文件后，后续补丁和保存都会以 SHA 冲突失败，不能悄悄重基线并覆盖外部内容。

## 0.9.31 - 2026-09-07

- 33 个开发系统统一为一个用户可见流程：人工编辑与系统 AI 共用应用私有工作副本，顶部直接保存、查看修改或撤回上次保存，不再向用户暴露 Draft/Apply 流程。
- 每次单系统或跨系统原子保存创建一个保存节点；恢复前校验当前磁盘 SHA，恢复本身追加节点，并保留固定领域包版本、权限、校验、事务 Journal 与崩溃恢复。
- MIR3 MCP 对外改用 Working Copy 工具名和 `workingCopyId`，旧 Draft 名称仅作为隐藏协议兼容层；Core Plugin 升级至 1.3.4。
- 修复保存节点启动恢复遇到另一实例的瞬时项目锁时错误进入永久只读的问题。
