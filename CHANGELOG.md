...
- 修复既有安装只暂存、不启用新领域包的问题：通过既有原子生命周期升级随包发布的版本，保留用户禁用状态、历史版本和回滚指针，33 个工具恢复官方文件投影。
- Harness 通过公开 XLS Viewer 扩展打开 Studio 表格编辑器，与开发工具复用同一编辑器、Working Copy、SHA 冲突检查和保存/撤回服务；未归属二开 XLS 使用人工编辑专用范围，不扩大系统 AI 权限。
- 原始 BIFF8 单元格保存校验文件格式和授权工作表/列，不再把缺少官方列映射的抽象业务 Schema 错当成实际表结构；业务语义校验未覆盖的范围在校验报告中明确标注。
- 删除生产路径的旧关键词归属回退；文件树加载失败明确显示错误，不再伪装成没有关联文件。Core Plugin 升级至 1.3.7。
- Core 验收统一消费已校验的 MessagePort 消息流，修复只监听旧窗口消息导致验收回复丢失；无项目首次启动延后验收，不误判为 Core 损坏。
