...
> 当前版本为 `0.9.25`。MIR3 Studio 通过统一系统开发内核、33 个领域包、归档系统会话和安全 Draft 工作流开发由 996 项目管理器创建的项目。
...
- 十六项固定 MCP 工具包括十二项领域工具与四项 GUI 工具：领域工具向系统 AI 和全局 Harness 暴露资源、依赖、Draft Diff、校验和能力目录，GUI 工具只操作 Studio 私有 Working Workspace。
...
- GUI Designer 的 AI 修改先进入应用私有 Working Workspace，不直接写游戏文件；只有用户点击“保存”或“撤回上次保存”才通过 SHA 冲突校验和保存节点更新 `客户端/dev/GUIExport`。
