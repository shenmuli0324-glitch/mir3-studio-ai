...
> 当前版本为 `0.9.33`。MIR3 Studio 通过统一系统开发内核、33 个领域包、归档系统会话和安全工作副本开发由 996 项目管理器创建的项目。
...
33 个领域包使用证据门控的引擎自动泛化：只归一化 SemVer、`v` 前缀 SemVer 与 `major.minor` 三类明确别名，并同时要求真实 996 项目目录、领域选择器/内容指纹与资源 Schema 校验。未识别或不兼容的引擎只能查看和诊断，工作副本写入与最终保存都会拒绝。
